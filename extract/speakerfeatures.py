...
def extract_13mfcc_13delta_13deltadelta(audio, rate):
    mfcc_feat = mfcc.mfcc(audio, rate, 0.025, 0.01, 13, appendEnergy=True)
    mfcc_feat = preprocessing.scale(mfcc_feat)
    delta = mfcc.delta(mfcc_feat, 2)
    delta_delta = mfcc.delta(delta, 2)
    all_feat = np.hstack((mfcc_feat, delta, delta_delta))
    return all_feat
